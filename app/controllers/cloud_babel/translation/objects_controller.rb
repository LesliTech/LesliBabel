...
module CloudBabel
    class Translation::ObjectsController < ApplicationController
        ...
        def index
            objects = Translation::Module.find(params[:module_id]).objects
            respond_to do |format|
                format.html { }
                format.json { responseWithSuccessful(objects) }
            end
        end

        # GET /translation/objects/1
        def show
            objects = Translation::Module.find(params[:module_id]).objects.find_by_id(params[:id])
            objects = Translation::Object.find(params[:id]) if objects.nil?
            respond_to do |format|
                format.html { }
                format.json { responseWithSuccessful(objects.children) }
            end
        end
